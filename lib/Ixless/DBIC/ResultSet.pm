...
use Ixless::Util qw(parsedate parsepgdate differ);
...
L<Ixless::DBIC::Result> rclasses, an L<Ixless::Processor::JMAP> object generates
...
interesting behavior is exposed in the L<Ixless::DBIC::Result> classes you write
for your application. Its methods are all called automatically by the handlers
generated by L<Ixless::Processor::JMAP> for your rclasses.  he only method from
...
sub _ix_rclass ($self) {
  ...
  Carp::confess("called with non-Ix-compatible result class: $rclass")
    unless $rclass->isa('Ixless::DBIC::Result');
...
needed (or allowed), then returns an L<Ixless::Result> object with the results.
...
sub ix_get ($self, $ctx, $arg = {}) {
  ...
  return $ctx->txn_do(sub {
    ...
    state $bad_idstr = Ixless::Validators::idstr();
    my @ids;
...
since the given C<sinceState>. In any case, this returns an C<Ixless::Result>
object.
...
sub ix_changes ($self, $ctx, $arg = {}) {
  ...
  return $ctx->txn_do(sub {
    ...
    die "wtf happened" unless $statecmp->$_isa('Ixless::StateComparison');
...
sub ix_purge ($self, $ctx, $arg = {}) {
  ...
  return $ctx->txn_do(sub {
    ...
    my $since = Ixless::DateTime->from_epoch(epoch => time - 86400 * 7);
...
generate the L<Ixless::Result> in response to the Foo/set call.
...
sub ix_create ($self, $ctx, $to_create) {
  ...
  TO_CREATE: for my $id (@keys) {
    ...
    my ($row, $error) = try {
      ...
      return (undef, $exception) if $exception->$_DOES('Ixless::Error');
  ...
  PROP: for my $prop (keys %props) {
    ...
    if ($date_fields{$prop}) {
      ...
      if (defined $value) {
        if (! ref $value) {
        ...
        } elsif (! $value->$_isa('Ixless::DateTime')) {
          ...
          $value = Ixless::DateTime->from_epoch(
            epoch => $value->epoch,
            time_zone => 'UTC'
          );
        }
...
returns a hashref that is eventually used to generate the L<Ixless::Result> in
  ...
  DESTROY: for my $id ($to_destroy->@*) {
    ...
    my $ok = eval {
      $ctx->txn_do(sub {
        $row->update({
          modSeqChanged => $next_state,
          dateDestroyed   => Ixless::DateTime->now,
...
L<Ixless::Result::FoosSet> result objects. (They're run in this order so that
...
sub ix_set ($self, $ctx, $arg = {}) {
  ...
  return $ctx->txn_do(sub {
    ...
    my $ret = [ Ixless::Result::FoosSet->new({
...
generates a resultset representing the search, and returns an L<Ixless::Result>
